...
export interface UserProfile {
    ...
    avatar: string;
    bio: string;
    npcNetwork?: NpcNetworkEntry[];    /** 分角色聊天头像（档案 App 设置）：charId → 头像（http(s) URL 或 data:image）。
...
export interface FullBackupData {
    ...
    /** 自主后端连接配置（utils/backendClient.ts 的 localStorage 配置随备份走）。 */
    backendChatConfig?: {
        enabled: boolean;
    ...
    novels?: NovelBook[];
    /** 自主后端：离线记忆同步队列 + 后端事件本地镜像（IndexedDB backend_sync_queue / backend_events store 的导出）。 */
