...
// JSZip 动态加载 / 导入中断标记辅助已拆到 utils/backupSystem.ts;此处 re-export 保住旧导入路径。
export { IMPORT_IN_PROGRESS_KEY, loadJSZip } from '../utils/backupSystem';
export type { ImportProgressUpdate } from '../utils/backupSystem';
...
export const OSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // 静默推到云端。失败只 console.warn——自动链路不打扰用户。
  // v3：普通数据（角色/聊天/设置）明文推送（RLS 行级隔离），API 密钥等敏感字段
  // 拆出后用本机记住的可复用密钥（登录时派生、extractable:false 存 IndexedDB）加密，
  // 全程不需要用户输密码。
  const cloudAutoSyncReady = () => {
      try {
          ...
          // autoSync 默认开：只有显式关过（false）才不自动同步
          return Boolean(sess?.accessToken && cfg?.autoSync !== false);
      } catch { return false; }
  };
      ...
      const handler = () => {
          ...
          void (async () => {
              try {
                  ...
                  // 隔 30 秒防抖：hidden/visible 抖动不连环触发
                  setTimeout(() => { cloudAutoSyncRef.current = false; }, 30_000);
              }
          })();
      };
      const onHide = () => { (window as any).__cloudPageHide = true; handler(); };
  ...
  // 与本机上次同步时间，云端更新（别台设备推过）→ 自动静默拉回合并，API 密钥用本机
  // 记住的钥匙一并解密（无需输密码）。冲突策略保守：只有云端更新才动本地，本机的
  // 新数据在页面隐藏时自动推上去，两台设备轮流交替不会互相覆盖丢失。
  // addToast / importSystem 定义在组件体后段——经 ref 取最新版，避免 use-before-declaration。
  ...
  // --- 备份导出/导入/重置:实现体在 utils/backupSystem.ts(依赖注入),此处只组 deps 并转发 ---
  ...
  const importSystem = (fileOrJson: File | string) => importSystemImpl({
    ...
    appearancePresets, characters, groups, worldbooks, novels, cloudBackupConfig, customThemes,
    userProfile, setSysOperation, addToast,
    updateTheme, updateApiConfig, updateRealtimeConfig, updateMemoryPalaceConfig,
    saveModels, savePresets, setCharacters,
    setGroups: importSystemSetGroups,
    setWorldbooks, setNovels, setSongs,
